...
### [v#{ENV['PRODUCT_VERSION']}](https://github.com/realityforge/gwt-ethereum-qr-code/tree/v#{ENV['PRODUCT_VERSION']}) (#{ENV['RELEASE_DATE']})
[Full Changelog](https://github.com/realityforge/gwt-ethereum-qr-code/compare/v#{ENV['PREVIOUS_PRODUCT_VERSION']}...v#{ENV['PRODUCT_VERSION']})
HEADER
...
HEADER
    ...
    stage('GithubRelease', 'Create a Release on GitHub') do
      ...
      client.login
      client.create_release('realityforge/gwt-ethereum-qr-code', tag, :name => tag, :body => changes, :draft => false, :prerelease => true)

      candidates = client.list_milestones('realityforge/gwt-ethereum-qr-code').select {|m| m[:title].to_s == tag}
      unless candidates.empty?
        milestone = candidates[0]
        unless milestone[:state] == 'closed'
          client.update_milestone('realityforge/gwt-ethereum-qr-code', milestone[:number], :state => 'closed')
        end
      end
    end
  end
